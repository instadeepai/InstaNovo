...
def aa_match_prefix(
    ...
    while i1 < len(peptide1) and i2 < len(peptide2):
        ...
        if abs(mass_diff(cum_mass1 + aa_mass1, cum_mass2 + aa_mass2, True)) < cum_mass_threshold:
            aa_matches[max(i1, i2)] = abs(mass_diff(aa_mass1, aa_mass2, True)) < ind_mass_threshold
            i1, i2 = i1 + 1, i2 + 1
...
def aa_match(
    ...
    if mode == "best":
        ...
        return aa_match_prefix(peptide1, peptide2, aa_dict, cum_mass_threshold, ind_mass_threshold)
    elif mode == "backward":
        aa_matches, pep_match = aa_match_prefix(
