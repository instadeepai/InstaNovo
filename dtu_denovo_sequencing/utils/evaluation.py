...
@click.option(
    "--peak_path",
    required=True,
    help="The file path with peak files for predicting peptide sequences or " "training Casanovo.",
)
@click.option(
    "--peak_path_val",
    ...
    if model is None and mode != "train":
        try:
            ...
            raise PermissionError(
                "GitHub API rate limit exceeded while trying to download the " "model weights"
            ) from None
...
def _get_model_weights() -> str:
    ...
    version_match: tuple[str | None, str | None, int] = None, None, 0
    # Try to find suitable model weights in the local cache.
    for filename in os.listdir(cache_dir):
        root, ext = os.path.splitext(filename)
        if ext == ".ckpt":
            file_version = tuple(g for g in re.match(r".*_v(\d+)_(\d+)_(\d+)", root).groups())
            match = sum(m) if (m := [i == j for i, j in zip(version, file_version)])[0] else 0
    ...
    # Otherwise try to find compatible model weights on GitHub.
        ...
        for release in repo.get_releases():
            ...
            match = sum(m) if (m := [i == j for i, j in zip(version, rel_version)])[0] else 0
            if match > version_match[2]:
        ...
        if version_match[2] > 0:
            ...
            with tqdm.tqdm.wrapattr(r.raw, "read", total=file_size, desc=desc) as r_raw, open(
                filename, "wb"
            ) as f:
