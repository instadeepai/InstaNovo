...
class TransNovo(nn.Module):
    def __init__(
        ...
        causal_decoder = CausalTransformerDecoder(
            CausalTransformerDecoderLayer(cfg.dim, cfg.nheads), cfg.layers
        )
        self.transformer = nn.Transformer(
            cfg.dim,
            cfg.nheads,
            cfg.layers,
            custom_decoder=causal_decoder,
            batch_first=True,
        )
        # self.transformer = nn.Transformer(H, A, L, batch_first=True)
    ...
    def forward(
        ...
        out = self.transformer(
            x,
            y,
            # tgt_mask=generate_square_subsequent_mask(y.shape[1]).to(y.device),
...
class CausalTransformerDecoder(nn.TransformerDecoder):
    ...
    def forward(
        ...
        # return output.transpose(0,1), new_cache
        return output.transpose(0, 1)
