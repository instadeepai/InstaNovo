...
class BaseDecoder:
    ...
    @abstractmethod
    def decode(
        self, spectra: Any, precursors: Any, spectra_mask: Any
    ) -> Tuple[Any, Any]:
...
class GreedyDecoder(BaseDecoder):
    ...
    def score(
        ...
        with torch.no_grad():
            logits = self.model(
                spectra, precursors, peptide, spectra_mask, y_mask, add_bos=True
            )
    ...
    def greedy(
        ...
        for _ in range(self.max_length):
            with torch.no_grad():
                logits = self.model(
                    spectra, precursors, y, spectra_mask, y_mask, add_bos=False
                )
