...
@patch("instanovo.utils.s3.S3FileHandler.s3_enabled", return_value=True, autospec=True)
@patch("instanovo.utils.s3.S3FileHandler._create_s3fs", autospec=True)
...
@patch("instanovo.utils.s3.S3FileHandler.s3_enabled", return_value=True, autospec=True)
@patch("instanovo.utils.s3.S3FileHandler._create_s3fs", autospec=True)
def test_s3_file_handler_get_local_path_missing(mock_create_s3fs: Any, mock_s3_enabled: Any) -> None:
...
@patch("instanovo.utils.s3.S3FileHandler.s3_enabled", return_value=True, autospec=True)
@patch("instanovo.utils.s3.S3FileHandler._create_s3fs", autospec=True)
def test_s3_file_handler_get_local_path_missing_ok(mock_create_s3fs: Any, mock_s3_enabled: Any) -> None:
...
@patch("instanovo.utils.s3.S3FileHandler._aichor_enabled", return_value=True, autospec=True)
...
def test_s3_file_handler_convert_to_s3_output(mock_aichor_enabled: Any) -> None:
    ...
    result = S3FileHandler.convert_to_s3_output("/local/path")
    # Normalize path separators for cross-platform compatibility
    result = result.replace("\\", "/")
    assert result == "s3://output/local/path"


# NO LONGER USED, TODO LATER?
...
#     assert S3FileHandler._clean_filepath(filepath) == "somefilepathwithchars-epoch2000.txt"
...
@patch.dict(
    os.environ,
    {
        "S3_ENDPOINT": "example/end_point",
        "AWS_ACCESS_KEY_ID": "example/key",
        "AWS_SECRET_ACCESS_KEY": "example/secret",
    },
)
@patch("s3fs.core.S3FileSystem", autospec=True)
