...
def test_upload(mock_open_local: Any, mock_create_s3fs: Any, mock_s3_enabled: Any) -> None:
...
def test_download(mock_open_local: Any, mock_create_s3fs: Any, mock_s3_enabled: Any) -> None:
...
def test_get_checkpoint_path(mock_download: Any, mock_makedirs: Any, mock_s3_enabled: Any) -> None:
    ...
    mock_download.assert_called_once_with("/input/model.ckpt", target="checkpoints/model.ckpt")
...
def test_s3_enabled() -> None:
    ...
    with patch.dict(os.environ, {"AICHOR_LOGS_PATH": "example/aichor/path"}, clear=True):
